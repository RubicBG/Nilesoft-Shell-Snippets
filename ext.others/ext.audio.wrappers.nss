...
menu(title='Audio Wrappers' image=image.fluent(\uF0EE,13)){
    ...
    admin cmd-ps=`-noexit $releaseInfo = Invoke-RestMethod -Uri "https://api.github.com/repos/kcat/openal-soft/releases/latest"; $DownloadURL = ($releaseInfo.assets | Where-Object { $_.name -like '*.zip' }).browser_download_url; $TempZipPath = '@sys.temp\\OpenALSoft.zip'; $ExtractPath = '@ds_path\\'; @dwnld_scr; if(Test-Path -LiteralPath "$ExtractPath\\OpenALSoft"){Remove-Item -LiteralPath "$ExtractPath\\OpenALSoft" -Recurse -Force}; ls -Path $ExtractPath -Filter 'openal-soft*' -Directory | Rename-Item -NewName 'OpenALSoft' -Force` wait=1})
