...
menu(title={ flags_m_str=reg.get(flags_m_reg, flag_file_path) flags_u_str=reg.get(flags_u_reg, flag_file_path) } + 'Compatibility'
	mode='single' type='file' where=reg.exists('HKCR\@reg('HKCR\@path.file.ext(flag_file_path)')\shellex\ContextMenuHandlers\Compatibility') image=icon.res('shell32.dll', 94)) {
