...
menu(title='UniExtract' type='file' image=cmd_UniExtract)
{
	...
	item(vis=!path.exists(cmd_UniExtract) title ='Download UniExtract' commands {
		...
		admin cmd-ps=`$DownloadURL = 'https://github.com/Bioruebe/UniExtract2/releases/download/v2.0.0-rc.3/UniExtractRC3.zip'; $TempZipPath = '@sys.temp\UniExtractRC3.zip'; $ExtractPath = '@path.parent(path.parent(cmd_UniExtract))'; if (!(Test-Path -Path $ExtractPath)) { New-Item -ItemType Directory -Path $ExtractPath -Force }; Invoke-WebRequest -Uri $DownloadURL -OutFile $TempZipPath; Expand-Archive -Path $TempZipPath -DestinationPath $ExtractPath -Force; Remove-Item -Path $TempZipPath -Force` wait=1,
